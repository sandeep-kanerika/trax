...
public class RateTableController {
	...
	@PostMapping(value = "/rate/tables", consumes = { MediaType.APPLICATION_JSON_VALUE }, produces = {MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<Object> createRateTables(@RequestBody RateTableVo rateTableVo) throws Exception {
		...
		if (existingRateSet == null) {
			throw new ResourceNotFoundException("Rateset not exist with id " + rateTableVo.getRateSetId());
		}
	...
	@PutMapping(value = "/rate/tables", consumes = { MediaType.APPLICATION_JSON_VALUE }, produces = {MediaType.APPLICATION_JSON_VALUE })
	...
	@DeleteMapping(value = "/rate/tables/{id}")
	public Boolean deleteRateTables(@PathVariable String id) {

		log.info("deleteRateTable invoked");
		return rateTablesService.deleteById(id);
	}
